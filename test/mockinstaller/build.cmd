...
:msi
	...
	echo [+] Compiling %1
	"%WIX%bin\candle" %WIX_CANDLE_FLAGS% -dPlatform=%1 -out "%~1\MozillaMockVPN.wixobj" -arch %1 MozillaMockVPN.wxs || exit /b %errorlevel%
	echo [+] Linking %1
	"%WIX%bin\light" %WIX_LIGHT_FLAGS% -out "%~1/MozillaMockVPN.msi" "%~1\MozillaMockVPN.wixobj" || exit /b %errorlevel%
	goto :eof
